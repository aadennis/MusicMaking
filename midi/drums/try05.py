...
def append_note_to_sequence(stream_obj, note):
    """
    Append a single note with velocity to a stream.
    
    Args:
        stream_obj: A music21 Stream object to append to
        note: the Music21 note to append
    """
    stream_obj.append(copy.deepcopy(note))
    return stream_obj
...
for index, row in notes.iterrows():
    ...
    append_note_to_sequence(stream1, create_note_with_velocity(note_obj, velocity))
    print(f"note: {note_obj}, velocity: {velocity}")

# Write to MIDI file
stream1.write('midi', fp="c:/temp/mahtest5.mid")
